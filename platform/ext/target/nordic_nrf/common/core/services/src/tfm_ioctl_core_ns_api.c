...
enum tfm_platform_err_t tfm_platform_sys_event_low_latency_on(int32_t *result)
{
	struct tfm_sys_event_service_args_t args = {
		.op = TFM_SYS_EVENT_OP_LOW_LATENCY_ON,
	};
...
{
	struct tfm_sys_event_service_args_t args = {
		.op = TFM_SYS_EVENT_OP_LOW_LATENCY_OFF,
	};
							       ...
							       int32_t *result)
{
	...
	args.rramc_dppi_channel = (uint32_t)ch;
